...
const startMarker = `  const isMizusawa = race.venue?.includes("水沢") || race.trackName?.includes("水沢") || race.raceName?.includes("水沢");
  if (isMizusawa) {
    tags.push("🐎 水沢特化OMEGAエンジン適用中");`;

const endMarker = `    // 岩手リーディング全般のフォロー(残りのトップ騎手)
    const isIwateEliteJ = ["山本聡", "高松亮", "菅原辰"].some(j => jockey.includes(j));`;

const newBlock = `    // --- 水沢特化 減点方式ロジックデータ (AIナレッジ完全準拠) ---
    // 初期スコアはベース100点からスタート
    ...
    const isDarkHorseExempt = isDarkHorse && isSakaiOrKobayashi && !isJockeyChangedMiz;
    ...
    } else if (cleanPrevJockeyMiz === cleanCurrentJockeyMiz) {
    ...
    if (frame >= 1 && frame <= 5) {
      ...
      if (frame === 8) tags.push("🎯 水沢鉄板: 迷ったら8枠！大特注の連軸候補");
    }
    ...
    if (prevRaceData?.last3F) {
      // 実際には最速かどうかは他馬と比較が必要ですが、速いタイム(36秒台等)なら減点またはフラグで処理
      // ここでは仕様通り一律で過信禁物タグと上位人気なら減点
      if (popularity <= 3) {
        potential -= 10;
        tags.push("🔻 水沢減点: 上がり最速の過信(-10点)");
      }
    }
