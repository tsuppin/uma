...
def main():
    print("Starting training process for Local Keiba...")
    ...
    # 過去成績（前走着順）の計算 (地方競馬は前5走まで)
    print("Calculating past race results (up to 5 races)...")
    ...
    # 2. 地方競馬のフィルタリング
    local_venues = ['笠松', '名古屋', '金沢', '盛岡', '水沢', '浦和', '船橋', '大井', '川崎', '帯広', '門別', '園田', '姫路', '高知', '佐賀']
    initial_len = len(df)
    df = df[df['会場'].isin(local_venues)]
    print(f"Filtered Local Keiba venues: {len(df)} / {initial_len} rows retained.")
    ...
    # モデル保存ディレクトリの作成
    save_dir = 'latest_models/local_keiba'
    ...
    print("\nSuccess! All models trained and saved to 'latest_models/local_keiba'.")
