...
for (let i = 0; i < lines.length; i++) {
  ...
  if (isResultSection) {
    ...
    // 着順の行をブロック開始とする (例: "1	枠7橙	14	ダノンプレサージュ...")
    // "13	枠8桃	15	" のように馬名がなくてもマッチする
    if (/^\d+\s*[\t　]+\s*枠\d/.test(l)) {
      blockStarts.push(i);
    }
  }
...
for (let bi = 0; bi < blockStarts.length; bi++) {
  ...
  const end = (bi < blockStarts.length - 1) ? blockStarts[bi+1] : start + 10;
  const blockLines = lines.slice(start, end).filter(l => l !== "");
  
  // blockLines をすべてタブとスペースでつなげてしまう？
  // いや、タブで split してから不要なものを捨てる方が安全。
  const combined = blockLines.join("\t").split(/\t+/).map(s => s.trim()).filter(s => s !== "");
  
  // combined の内容例（パターン1）:
  // [ '1', '枠7橙', '14', 'ダノンプレサージュ', '牝3', '55.0', '横山 典弘', '1:36.5', '2 2', '36.4', '488(+2)', '大竹 正博', '1' ]
  
  // combined の内容例（パターン2: 13着）:
  // [ '13', '枠8桃', '15', 'マッドヘッドラブブリンカー着用', '牝3', '55.0', '武藤 雅', '1:39.7', 'クビ', '5 5', '39.1', '476(+12)', '戸田 博文', '10' ]
