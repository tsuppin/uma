...
const endTarget = `    // 1. 芝マイル以下（1400m・1600m）：「先行〜中団差し」×「圧倒的な内枠（2〜3枠）」`;
...
`;
  ...
  const newContent = content.substring(0, startIndex + startTarget.length) + insertStr + content.substring(endIndex);
  fs.writeFileSync(filePath, newContent, 'utf8');
  console.log("Successfully inserted Tokyo 5 rules.");
} else {
  console.log("Error finding targets:", startIndex, endIndex);
}
