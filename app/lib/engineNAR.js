...
exports.calculateNARScore = calculateNARScore;
var waveLevelCalculator_1 = require("./waveLevelCalculator");
// 地方特化のエリート騎手リスト（南関東を中心とした地方トップジョッキー）
var NAR_ELITE_JOCKEYS = ["森泰斗", "御神本訓史", "矢野貴之", "笹川翼", "吉原寛人", "和田譲治", "山崎誠士", "吉村智洋", "赤岡修次", "山口勲"];
// 地方ダート無双血統
var NAR_POWER_SIRES = ['シニスターミニスター', 'パイロ', 'サウスヴィグラス', 'ヘニーヒューズ', 'マジェスティックウォリアー', 'エスポワールシチー', 'ホッコータルマエ', 'コパノリッキー', 'スマートファルコン', 'カネヒキリ'];
function calculateNARScore(horse, race, learningPatches, masterData) {
    ...
    // ==========================================
    // 地方特化のパワー血統加点
    var isNarSire = NAR_POWER_SIRES.some(function (s) { return (horse.sire || bloodline).includes(s); });
    if (isNarSire) {
        // [減点方式] potential += 40;
        // [要見直し] tags.push("👑 地方ダート特注: 深い砂を力でねじ伏せる圧倒的パワー血統");
    }
    // 地方エリート騎手加点
    if (NAR_ELITE_JOCKEYS.some(function (j) { return jockey.includes(j); })) {
        // [減点方式] potential += 35;
    ...
    if (trackName.includes('浦和')) {
        ...
        // ==========================================
        var popularity_1 = horse.popularity || 99;
        var weightChange = horse.weightChange || 0;
        var prevRace = horse.pastRaces && horse.pastRaces.length > 0 ? horse.pastRaces[0] : null;
        var isJpnGrade = race.raceName && race.raceName.match(/Jpn[1-3I-III]/i);
        var is3yoRace = race.raceName && race.raceName.includes('3歳');
        var isKobaRace = !is3yoRace && !isJpnGrade && race.raceClass && race.raceClass.match(/[ABC]級/i);
        ...
        var prevRaceJockey = prevRace ? prevRace.jockey : horse.prevJockey;
        ...
        if (horse.coatColor && horse.coatColor !== '鹿毛' && horse.coatColor !== '黒鹿毛') {
            potential -= 5;
            tags.push("\u274C \u5927\u4E95\u6E1B\u70B9: \u512A\u52E2\u6BDB\u8272\u4EE5\u5916\u306E\u6BDB\u8272(".concat(horse.coatColor, ")\u30DA\u30CA\u30EB\u30C6\u30A3"));
        }
        // ■ 条件別・レース別減点基準
        ...
        var isLatterHalf = race.raceNumber && race.raceNumber >= 6;
        ...
        if (isJpnGrade) {
            var isJRAHorse = ((_b = horse.belonging) === null || _b === void 0 ? void 0 : _b.includes('JRA')) || ((_c = horse.stableLocation) === null || _c === void 0 ? void 0 : _c.match(/(美浦|栗東)/)) || (horse.jockey && ['ルメール', '川田', '武豊', '戸崎', '松山', '坂井', '横山武', '岩田望', '西村淳'].some(function (j) { return horse.jockey.includes(j); }));
            if (!isJRAHorse) {
                potential -= 30;
        ...
        // ==========================================
        var popularity_2 = horse.popularity || 99;
        var prevRaceData_1 = horse.pastRaces && horse.pastRaces.length > 0 ? horse.pastRaces[0] : undefined;
        // ※川崎特化：基本スコアを100点からスタートし、減点方式で評価する
        potential = 100;
        var isHeavyTrack = race.condition === '重' || race.condition === '不良' || race.condition === '稍重';
        var jName_1 = horse.jockey || '';
        // --- [減点ルール] -------------------------------------
        ...
        if (jName_1.includes('☆') || jName_1.includes('△') || jName_1.includes('▲') || jName_1.includes('◇') || jName_1.includes('★')) {
            potential -= 25;
        ...
        if (isHeavyTrack && dist !== 900) {
            if (horse.style === '差し' || horse.style === '追込' || horse.style === '後方') {
                var hasFastest3f = false;
                if (horse.pastRaces) {
                    for (var _i = 0, _h = horse.pastRaces; _i < _h.length; _i++) {
                        var r = _h[_i];
                        if (r.last3fTime) {
                            var last3f = parseFloat(r.last3fTime);
        ...
        if (prevRaceData_1 && prevRaceData_1.result >= 6) {
            var isExempt = false;
            // 免除条件1: 近5走以内に「川崎コース」での好走歴（3着以内）
            if (horse.pastRaces) {
                for (var i = 0; i < Math.min(horse.pastRaces.length, 5); i++) {
                    var r = horse.pastRaces[i];
                    if (r.venue && r.venue.includes('川崎') && r.result <= 3)
                        isExempt = true;
                }
            }
            // 免除条件2: JRAからの転入初戦、または他場での重賞実績
            var isFromJRA = horse.transferFrom === 'JRA' || horse.belonging === 'JRA';
            var hasGradedStakes = false;
            if (horse.pastRaces) {
                for (var _j = 0, _k = horse.pastRaces; _j < _k.length; _j++) {
                    var r = _k[_j];
            ...
            var isJockeyChangedK = false;
            if (prevRaceData_1.jockey && (!jName_1.includes(prevRaceData_1.jockey) && !prevRaceData_1.jockey.includes(jName_1))) {
                isJockeyChangedK = true;
            }
            if (isJockeyChangedK && ['町田', '矢野', '澤田', '西啓太'].some(function (j) { return jName_1.includes(j); })) {
                isExempt = true;
            }
            if (!isExempt) {
                potential -= 25;
        ...
        if (popularity_2 >= 6 && popularity_2 <= 8) {
            if (prevRaceData_1 && prevRaceData_1.result >= 6) {
                potential -= 20;
        ...
        // ==========================================
        var popularity_3 = horse.popularity || 99;
        var prevRace = horse.pastRaces && horse.pastRaces.length > 0 ? horse.pastRaces[0] : null;
        var kasamatsuPenalty = 0;
        var weightChange = horse.weightChange || 0;
        ...
        // 軸馬は「1・2番人気」、相手には「中穴」
        if (popularity_3 === 1 || popularity_3 === 2) {
            potential += 30;
            tags.push("👑 笠松鉄板: 信頼度の高い上位人気(1・2番人気軸)");
        }
        else if (popularity_3 >= 4 && popularity_3 <= 7) {
            potential += 15;
            tags.push("🌟 笠松特注: ヒモ荒れを演出する中穴候補(4〜7番人気必須)");
        }
        else if (popularity_3 >= 8) {
        ...
        if (horse.jockey) {
            ...
            var isApprenticeKasamatsu = horse.jockey.match(/[☆▲△◇]/);
        ...
        // 特別戦の「笹野博厩舎 × 渡邊竜也騎手」
        var isSpecialRace = race.raceName && race.raceName.includes('特別');
        ...
        // 初期スコアを「100点」にリセット
        potential = 100;
        var popularity_4 = horse.popularity || 99;
        var prevRaceData_2 = horse.pastRaces && horse.pastRaces.length > 0 ? horse.pastRaces[0] : undefined;
        // -----------------------------------------------------
        // 【1. 基本能力・適性による減点】
        // -----------------------------------------------------
        // 人気減点【-10点】：当日「4番人気以下」
        ...
        if (prevRaceData_2 && prevRaceData_2.distance !== undefined && prevRaceData_2.distance !== dist) {
            potential -= 10;
        ...
        // -----------------------------------------------------
        var top3Count_1 = 0;
        var allWorseThan6_1 = true;
        var olderGoodRun_1 = false;
        var recentSlump_1 = false;
        if (horse.pastRaces && horse.pastRaces.length > 0) {
            var recentRaces = horse.pastRaces.slice(0, 5);
            recentRaces.forEach(function (pr, index) {
                if (pr.result && pr.result <= 3) {
                    top3Count_1++;
                    allWorseThan6_1 = false;
                    if (index >= 2) {
                        olderGoodRun_1 = true;
                    } // 3走前〜5走前に好走
                }
                ...
                if (index < 2 && pr.result && pr.result >= 6) {
                    recentSlump_1 = true; // 前走・前々走が6着以下
                }
            });
            // スランプ減点【-20点】：過去5走すべて6着以下
            if (allWorseThan6_1 && recentRaces.length >= 3) {
                potential -= 20;
                tags.push("⚠️ 園田消去法: 過去すべて6着以下の完全スランプ");
            }
        }
        // 前走着順減点【-10点】：前走4着以下（ただし過去5走中3回以上1-3着の馬は免除）
        if (prevRaceData_2 && prevRaceData_2.result >= 4) {
            if (top3Count_1 < 3) {
                potential -= 10;
        ...
        // 【3. 騎手・陣営による減点】
        // -----------------------------------------------------
        var jName_2 = horse.jockey || '';
        var tName_1 = horse.trainer || '';
        var isTopJockey = ['田野', '小牧太'].some(function (j) { return jName_2.includes(j); });
        // 騎手力減点【-15点】：田野豊・小牧太以外
        if (!isTopJockey) {
            potential -= 15;
        ...
        var isJockeyChanged = prevRaceData_2 && prevRaceData_2.jockey && jName_2 !== prevRaceData_2.jockey;
        if (popularity_4 <= 3 && isJockeyChanged && !isTopJockey) {
            potential -= 10;
            tags.push("⚠️ 園田減点: 上位人気馬の不穏な乗り替わり");
        }
        // 単騎出し減点【-5点】
        if (tName_1) {
            var sameTrainerCount = race.horses.filter(function (h) { return h.trainer && h.trainer.includes(tName_1); }).length;
        ...
        var raceNumMatch = race.raceName ? race.raceName.match(/(\d+)R/) : null;
        var raceNum = raceNumMatch ? parseInt(raceNumMatch[1], 10) : (race.raceNumber || 0);
        ...
        // -----------------------------------------------------
        if (popularity_4 >= 5) {
            var isHimoHole = false;
            var himoReason = "";
            // フラグA：前走凡走 × 3〜5走前に好走
            if (recentSlump_1 && olderGoodRun_1) {
                isHimoHole = true;
                himoReason = "隠れた実力(過去好走歴)";
            }
            // フラグB：前走後方待機、または距離短縮
            if (horse.style === '追込' || horse.style === '後方') {
                isHimoHole = true;
            ...
            if (prevRaceData_2 && prevRaceData_2.distance !== undefined && prevRaceData_2.distance > dist) {
                isHimoHole = true;
                himoReason = "大幅な距離短縮恩恵";
            }
            // フラグC：陣営の波乱使者（小谷哲 / 尾林二 / 碇清次）
            if (jName_2.includes('小谷') || ['尾林二', '碇清次'].some(function (t) { return tName_1.includes(t); })) {
                isHimoHole = true;
                himoReason = "波乱メーカー陣営";
            }
            // フラグD：恩恵乗り替わり（斤量1.0kg以上減）
            if (isJockeyChanged && prevRaceData_2 && prevRaceData_2.jockeyWeight !== undefined && horse.jockeyWeight !== undefined) {
                var prevW = prevRaceData_2.jockeyWeight;
                var currW = horse.jockeyWeight;
            ...
            // フラグE：本命馬と同枠・隣枠
            var favHorse = race.horses.find(function (h) { return h.popularity === 1; });
            if (favHorse && favHorse.frame && Math.abs(frame - favHorse.frame) <= 1) {
                isHimoHole = true;
                himoReason = "本命馬の同枠/隣枠";
            }
            if (isHimoHole) {
                // ヒモ穴として拾いやすくするため救済加点
                potential += 30;
                tags.push("\uD83D\uDCA5 \u5712\u7530\u7279\u6CE8: \u7D76\u597D\u306E\u30D2\u30E2\u7A74\u63A8\u5968\u30D5\u30E9\u30B0\u6210\u7ACB\uFF01(".concat(himoReason, ")"));
            }
        }
    }
        ...
        if (frame <= 3 && (horse.style === '逃げ' || horse.style === '先行')) {
            // [減点方式] potential += 45;
            tags.push("\uD83D\uDD25 ".concat(trackName.replace(/競馬場/g, ''), "\u30DE\u30CB\u30A2\u30C3\u30AF: \u8D85\u5C0F\u56DE\u308A\u30B3\u30FC\u30B9\u3067\u7269\u7406\u7684\u306B\u6B62\u307E\u3089\u306A\u3044\u300E\u5185\u67A0\u306E\u9003\u3052\u5148\u884C\u300F"));
        }
        if (horse.style === '追込') {
            potential -= 40;
            tags.push("\u26A0\uFE0F ".concat(trackName.replace(/競馬場/g, ''), "\u5371\u967A: \u30B3\u30FC\u30CA\u30FC\u304C\u30BF\u30A4\u30C8\u3059\u304E\u3066\u7269\u7406\u7684\u306B\u5C4A\u304B\u306A\u3044\u8FFD\u8FBC\u99AC"));
        }
    }
        ...
        // ==========================================
        var popularity_5 = horse.popularity || 99;
        var prevRace = horse.pastRaces && horse.pastRaces.length > 0 ? horse.pastRaces[0] : null;
        var mombetsuPenalty = 0;
        // Rule 1: 枠順ペナルティ（内枠）
        if (frame >= 1 && frame <= 4) {
            mombetsuPenalty += 15;
        ...
        var isApprentice = horse.jockey && horse.jockey.match(/[☆▲△◇]/);
        var isHimoJockey = horse.jockey && (horse.jockey.includes('服部茂') || horse.jockey.includes('岩橋勇'));
        ...
        if (popularity_5 === 1 || popularity_5 === 2) {
        ...
        if ((horse.style === '差し' || horse.style === '追込') && isNarSire) {
            potential += 15;
            tags.push("\uD83D\uDD25 ".concat(trackName.replace(/競馬場/g, ''), "\u30DE\u30CB\u30A2\u30C3\u30AF: \u5730\u65B9\u5C48\u6307\u306E\u5927\u7BB1\u30B3\u30FC\u30B9\u3067\u672B\u811A\u304C\u7206\u767A\u3059\u308B\u30D1\u30EF\u30FC\u578B\u5DEE\u3057\u99AC"));
        }
    }
    // ==========================================
    // ベースロジック（オッズ歪み等）
    ...
    // ===================================================
    var frontRunnersCount = race.horses.filter(function (h) { return h.style === '逃げ'; }).length;
    var isFrontBias = race.condition === '重' || race.condition === '不良' ||
        learningPatches.some(function (p) { return p.description.includes('前残り'); });
    ...
    if (horse.style === '先行' || horse.style === '好位') {
        var insideHorse = race.horses.find(function (h) { return h.number === horse.number - 1; });
    ...
    if (prevRaceData && prevRaceData.jockey) {
        var prevWasTop = NAR_ELITE_JOCKEYS.some(function (j) { return prevRaceData.jockey.includes(j); });
        var nowIsTop = NAR_ELITE_JOCKEYS.some(function (j) { return horse.jockey.includes(j); });
    ...
    if (horse.rotation === '連闘' && prevRaceData) {
        ...
        else if ((_e = (_d = masterData === null || masterData === void 0 ? void 0 : masterData.horses[horse.name]) === null || _d === void 0 ? void 0 : _d.incidents) === null || _e === void 0 ? void 0 : _e.some(function (i) { return i.note === "上がり最速で敗退"; })) {
            // [減点方式] potential += 15;
    ...
    if (!trackName.includes('大井') && horse.jockeyWeight && horse.jockeyWeight <= 53 && (horse.style === '逃げ' || horse.style === '先行')) {
        // [減点方式] potential += 20;
        tags.push("\uD83E\uDEBD \u88F8\u540C\u7136\u306E\u8EFD\u65A4\u91CF(".concat(horse.jockeyWeight, "kg): \u6E1B\u91CF\u9A0E\u624B\u00D7\u5148\u884C\u529B\u306B\u3088\u308B\u30A2\u30C9\u30D0\u30F3\u30C6\u30FC\u30B8"));
    }
    // 4. 南関東（エリート地区）からの遠征馬バイアス
    var isSouthKantoTrack = race.trackName.match(/(大井|川崎|船橋|浦和)/);
    if (!isSouthKantoTrack) {
        var isSouthKantoHorse = ((_f = horse.belonging) === null || _f === void 0 ? void 0 : _f.match(/(大井|川崎|船橋|浦和)/)) || ((_g = horse.stableLocation) === null || _g === void 0 ? void 0 : _g.match(/(大井|川崎|船橋|浦和)/));
    ...
    if (masterData && masterData.horses && masterData.horses[horse.name]) {
        ...
        if (historicalIncidents && historicalIncidents.length > 0) {
            ...
            var hasFastest3fLoss = historicalIncidents.some(function (inc) { return inc.note === "上がり最速で敗退"; });
            if (hasHugeWin) {
                // [減点方式] potential += 20;
    ...
    // 動的学習パッチの適用
    // ==========================================
    for (var _l = 0, learningPatches_1 = learningPatches; _l < learningPatches_1.length; _l++) {
        var patch = learningPatches_1[_l];
        ...
        for (var _m = 0, _o = patch.adjustments; _m < _o.length; _m++) {
            var adj = _o[_m];
            var field = adj.field;
            var val = horse[field];
            var applies = false;
    ...
    // ─────────────────────────────────────────
    var narSmallCourse = ['大井', '川崎', '浦和', '船橋', '高知', '笠松', '金沢', '佐賀'].some(function (t) { var _a; return (_a = race.trackName) === null || _a === void 0 ? void 0 : _a.includes(t); });
    ...
    // ─────────────────────────────────────────
    var narSmallTrack = ['浦和', '川崎', '笠松', '佐賀'].some(function (t) { var _a; return (_a = race.trackName) === null || _a === void 0 ? void 0 : _a.includes(t); });
    ...
    // 【NAR-E】前走大敗×低人気ペナルティ
    // ─────────────────────────────────────────
    var narPrevRace = horse.pastRaces && horse.pastRaces.length > 0 ? horse.pastRaces[0] : null;
    ...
    // ─────────────────────────────────────────
    var narSire = horse.sire || '';
    var isTurfOnlyNAR = ['ディープインパクト', 'コントレイル', 'エピファネイア'].some(function (s) { return narSire.includes(s); });
    if (isTurfOnlyNAR && race.surface === 'ダート') {
        potential -= 15;
        tags.push("❌ NAR血統不適ペナルティ: 芝専門血統は地方ダートで能力を出し切れないリスク");
    }
    var finalPotential = Math.round(Math.max(0, potential) * 10) / 10;
    var darkness = (finalPotential / 100) * Math.pow(odds, 1.1);
    return {
        horseId: horse.id,
