...
// [PERFORMANCE FIX]         tags.push(`📈 展開利: 前傾ラップ(H)予想で差し・追込台頭`);
  ...
  if (trackName.includes('東京')) {
    ...
    if (prevRaceData && prevRaceData.result >= 10 && popularity >= 5) {
      potential -= 30;
// [PERFORMANCE FIX]       tags.push("⚠️ 東京減点方式: 一変の要素なし。前走大敗かつ今回人気薄(5番人気以下)");
    }
    ...
    if (popularity >= 10) {
      potential += 15;
// [PERFORMANCE FIX]       tags.push("💥 東京新特注(13/14日分析): 大荒れ警戒・二桁人気のヒモ穴候補");
    }
    ...
    // 新ルール10: 1着（単勝・アタマ）を狙うなら「2枠」の馬を重視する
    // 2枠の馬は軸候補として大きく加点
    if (frame === 2) {
      potential += 25;
      tags.push("🔥 東京新特注(13/14日分析): 圧倒的勝率を誇る絶好枠「2枠」(アタマ狙い推奨)");
    }

    // 新ルール11: 大外の「8枠」は馬券の軸・ヒモとして信頼度が極めて高い
    // 8枠の馬は無条件で買い目に組み込むレベルで加点
    if (frame === 8) {
      potential += 20;
      tags.push("👑 東京新特注(13/14日分析): 軸・ヒモ共に信頼度MAXの「8枠」(無条件買い)");
    }

    // 新ルール12: 「最内（1枠）」は過信禁物、全体的に「外枠（6〜8枠）」優勢
    // 1枠は減点、6〜7枠は加点（8枠は上記で加点済み）
    if (frame === 1) {
      potential -= 20;
      tags.push("⚠️ 東京新特注(13/14日分析): 苦戦傾向の最内「1枠」(過信禁物・消し推奨)");
    } else if (frame === 6 || frame === 7) {
      potential += 15;
// [PERFORMANCE FIX]       tags.push("🎯 東京新特注(13/14日分析): 馬券に絡みやすい外枠優勢傾向(6〜7枠)");
    }
    ...
    if (prevRaceData && prevRaceData.result === 1) {
      potential += 20;
// [PERFORMANCE FIX]       tags.push("🔥 東京新特注(13/14日分析): 勢い止まらぬ前走1着馬(昇級の壁なし・軸推奨)");
    }
    ...
    if (isJockeyChangedNew) {
      // 基本乗り替わり加点
      potential += 15;
// [PERFORMANCE FIX]       tags.push("🔄 東京新特注(13/14日分析): 陣営の勝負気配漂う「乗り替わり」馬");
      ...
      if (popularity >= 10) {
        potential += 25;
// [PERFORMANCE FIX]         tags.push("💥 東京新特注(13/14日分析): 乗り替わり×二桁人気(一変・超大穴狙い)");
      }
    }
    ...
    if (isMixedRace || race.raceNumber >= 6) { // 混合戦や後半のレースを想定
      if (age === 3 || age === 4) {
        potential += 15;
// [PERFORMANCE FIX]         tags.push("🔥 東京新特注(13/14日分析): 充実期・成長期の若い馬(3歳/4歳)");
      }
    } else {
      if (age === 3 || age === 4) {
        potential += 10;
    ...
    // 新ルール34 & 減点5: 「6歳以上の高齢馬」を1着候補から外す（減点）
    if (age >= 6 && isMixedRace) {
      potential -= 10; // 減点方式仕様に準拠(-10点)
    ...
    if (typeof horse.weightChange === 'number') {
      ...
      if (horse.weightChange >= 10 && isLongRest) {
        ...
        potential += 25;
// [PERFORMANCE FIX]         tags.push("🔥 東京新特注(13/14日分析): 休養明けの大幅馬体増(+10kg〜)は完全な「成長分」(一変期待)");
        ...
        potential += 10;
// [PERFORMANCE FIX]         tags.push("👑 東京特注: 古馬の馬体重維持(±4kg以内)による安定感");
      }
      
      // 新ルール5 & 減点1: 馬体重の「大幅なマイナス（二桁減）」は危険信号
      // 当日-10kg以上の大幅馬体重減は調子落ちとして大きく減点する
      if (horse.weightChange <= -10) {
        potential -= 30; // 減点方式仕様に準拠(-30点)
        tags.push("⚠️ 東京新特注(13/14日分析): 大幅な馬体減(-10kg以上)は致命的(消し推奨)");
      }
    }

    // 新ルール28 & 減点6: 「ブリンカー着用」の複合条件（クロス分析）および「不発」判定
    if (horse.useBlinkers) {
      const isWeightHugeChange = typeof horse.weightChange === 'number' && Math.abs(horse.weightChange) >= 10;
      if (!isJockeyChangedNew || isWeightHugeChange) {
        potential -= 10; // 加点ゼロ ＆ さらに減点(-10点)
        tags.push("⚠️ 東京減点ロジック: 激走条件を満たさないブリンカー不発(陣営の空回り危険)");
      } else {
        potential += 20;
    ...
    if (age <= 4) {
      potential += 15;
// [PERFORMANCE FIX]       tags.push("🔥 東京新特注(13/14日分析): 成長著しい若駒(3〜4歳)の勢い重視(アタマ候補)");
    } else if (age >= 6) {
      potential -= 20;
// [PERFORMANCE FIX]       tags.push("⚠️ 東京新特注(13/14日分析): 6歳以上の高齢馬はアタマ(1着)・連軸としては危険");
    }
    ...
    if (prevRaceData && prevRaceData.result === 1) {
      potential += 15;
// [PERFORMANCE FIX]       tags.push("🔥 東京特注: 昇級戦の壁なし！前走1着の勢いそのまま連勝へ");
    }
    ...
    if (prevRaceData && prevRaceData.result >= 10) {
      // ヒモとして残すために加点
      potential += 15;
// [PERFORMANCE FIX]       tags.push("💥 東京特注: 前走大敗(二桁着順)からの豹変警戒(高配当の使者)");
    }
...
// [PERFORMANCE FIX]         tags.push("💥 東京特注: 大波乱の立役者！前走大敗からの乗り替わり一変警戒");
      }
    }
    ...
    if (age <= 3) {
      if (weightChange >= 10) {
        // [減点方式] potential += 40;
// [PERFORMANCE FIX]         tags.push("🔥 東京馬体重: 3歳馬の大幅プラス(成長・筋肉量UP)");
      }
    } else if (age >= 4) {
      if (weightChange >= -4 && weightChange <= 4) {
        // [減点方式] potential += 35;
// [PERFORMANCE FIX]         tags.push("👑 東京馬体重: 古馬の微増減(ベスト体重での仕上がり鉄板)");
    ...
    if (trackName.includes('東京')) {
      ...
      if (prevRaceData && (prevRaceData.direction === '右' || ['中山', '阪神', '京都', '福島', '小倉', '函館', '札幌'].some(t => prevRaceData.venue?.includes(t)))) {
        if (prevRaceData.result >= 4) {
          // [減点方式] potential += 35;
// [PERFORMANCE FIX]           tags.push("🔥 東京特注: 右回り惨敗からの左回り替わり(サウスポーの逆襲)");
        }
      }
  ...
  if (weight > 0) {
    ...
    if (kinryoWeightRatio < 11.5) {
      ...
      potential -= 30;
// [PERFORMANCE FIX]       tags.push("⚠️ 物理的過負荷:小柄馬の斤量負担ペナルティ");
    }
  }
  ...
  if (horseBreeder) {
    ...
    if (breederName.includes("ノーザンファーム")) {
      ...
      // [減点方式] potential += 8;
// [PERFORMANCE FIX]       tags.push(`🐎 有名実力牧場生産(ブランド力)`);
    }
  }
  ...
  if (isNiigata) {
// [PERFORMANCE FIX]     tags.push("🌾 新潟特化OMEGAエンジン適用中");
...
// [PERFORMANCE FIX]       tags.push("⚠️ 大幅馬体重増減（2・3着激走の紐穴期待値）");
      if (popularity >= 6 || odds >= 12.0) {
        // [減点方式] potential += 10;
// [PERFORMANCE FIX]         tags.push("⚡ 大幅増減・妙味穴馬補正");
      }
    }
    ...
    if (race.raceNumber >= 9) {
      ...
      if (isRitto) {
        // [減点方式] potential += 25;
// [PERFORMANCE FIX]         tags.push("✈️ メイン戦遠征関西馬(栗東)エッジ");
      }
      // エリート騎手
...
// [PERFORMANCE FIX]       tags.push("🎯 京都ブリンカー着用適正化");
    }
...
// [PERFORMANCE FIX]       tags.push("✈️ メイン戦遠征関西馬(栗東)エッジ");
    ...
    if (horse.pastRaces && horse.pastRaces[0]) {
      ...
      if (prevRace.surface === "芝") {
        // [減点方式] potential += 15;
// [PERFORMANCE FIX]         tags.push("🚀 東京ダート替わり：芝スタート芝ダッシュ期待馬");
      }
    }
    ...
    if (horse.pastRaces && horse.pastRaces[0]) {
      ...
      if (prevRace.distance > dist) {
        // [減点方式] potential += 12;
// [PERFORMANCE FIX]         tags.push("📈 距離短縮ローテ：タフな流れへのスタミナ適合");
      }
    }
    ...
    if (horse.pastRaces && horse.pastRaces.length > 0) {
      ...
      if (hasJraBigLoss) {
        potential += 35;
// [PERFORMANCE FIX]         tags.push("🌟 名古屋特注: JRA大敗組(地方馬との圧倒的な能力差で一変のサイン)");
      }
      ...
      if (is3yoRace) {
        ...
        if (isJraFirst) {
          potential += 40;
// [PERFORMANCE FIX]           tags.push("🔥 名古屋3歳鉄板: JRAからの転入初戦(地方勢との能力差で一変確実)");
      ...
      if (horse.gender === '牝') {
        potential += 15;
// [PERFORMANCE FIX]         tags.push("🎀 名古屋牝馬恩恵: 小回りコースでの斤量差アドバンテージ");
        const currentJockey = horse.jockey || '';
        const isCurrentClaiming = currentJockey.match(/[▲△☆◇]/);
    ...
    if (weightChange >= 0 && weightChange <= 6) {
      // [減点方式] potential += 25;
// [PERFORMANCE FIX]       tags.push("🏆 東京勝ち切り条件：馬体重安定ゾーン（±0〜+6kg）");
    } else if (absWeightChange <= 8) {
      // [減点方式] potential += 15;
...
// [PERFORMANCE FIX]         tags.push("⚡ 大幅増減・妙味穴馬補正");
  ...
  if (isTokyoSpecial) {
// [PERFORMANCE FIX]     tags.push("🐎 東京特化OMEGAエンジン適用中");
  ...
  if (hm) {
    ...
    if (hm.bestTime && hm.bestTime[key]) {
      ...
      // [減点方式] potential += 25;
// [PERFORMANCE FIX]       tags.push(`⌚ 生涯ベスト時計保有(スピード上位)`);
    }
  }
  ...
  if (horse.pastRaces && horse.pastRaces.length > 0) {
    ...
    if (pastWins > 0) {
      // [減点方式] potential += 20;
// [PERFORMANCE FIX]       tags.push('🤝主戦騎手(勝利実績)');
    } else if (pastTop3 > 0) {
      // [減点方式] potential += 10;
      tags.push('🤝主戦騎手(好走実績)');
  ...
  if (race.surface === 'ダート') {
    ...
    if (breederName.match(eliteDirtBreeders)) {
      // [減点方式] potential += 20;
// [PERFORMANCE FIX]       tags.push(`🏡ダート優秀牧場`);
    }
  } else if (race.surface === '芝') {
  ...
  if (bmsName) {
    ...
    if (race.surface === 'ダート') {
      ...
      if (bmsName.match(dirtEliteBMS)) {
        // [減点方式] potential += 20;
// [PERFORMANCE FIX]         tags.push(`💪 砂のスタミナ(母父): ダート適性に優れたBMS血統エッジ`);
      }
    }
...
// [PERFORMANCE FIX]         tags.push('🚀若駒成長分(3着内期待)');
      } else if (weightChange <= 16) {
        // [減点方式] potential += 10;
        tags.push('🚀馬体充実(ヒモ警戒)');
      } else {
      ...
      if (weightChange >= -18) {
        // [減点方式] potential += 15;
// [PERFORMANCE FIX]         tags.push('🎯究極の絞り(ヒモ荒れ注意)');
      } else {
        potential -= 30;
...
// [PERFORMANCE FIX]     tags.push('💎4枠:激走の紐穴(複勝率1位)');
  } else if (frame === 8) {
  ...
  if (horse.pastRaces && horse.pastRaces.length > 0) {
    ...
    if (tDiff < 0) {
      ...
      // [減点方式] potential += 25;
// [PERFORMANCE FIX]       tags.push('🔥王道パターン(前走好走/僅差)');
    }
    ...
    if (race.surface === '芝') {
      ...
      if (bestLast3f <= 38.2) {
        // [減点方式] potential += 35;
// [PERFORMANCE FIX]         tags.push(`💪ダート高速末脚(上がり最速級)`);
      ...
      if (race.raceNumber >= 8 && bestLast3f <= 40.5) {
        // [減点方式] potential += 30;
// [PERFORMANCE FIX]         tags.push(`🌃後半戦:安定した末脚`);
        if (bestLast3f <= 39.9) {
          // [減点方式] potential += 15;
// [PERFORMANCE FIX]           tags.push('🔥後半戦:39秒台の決定力');
        }
      }
    }
    ...
    if (hasFastAndLate) {
      // [減点方式] potential += 30;
// [PERFORMANCE FIX]       tags.push('🏆総合スピード能力(タイム×上がり相関)');
    }
  ...
  if (kinryo === 55) {
    ...
    if (race.raceNumber >= 7) {
      // [減点方式] potential += 20;
// [PERFORMANCE FIX]       tags.push('🏰重量実力馬(後半勝負)');
    } else {
      potential -= 15;
  ...
  if (race.surface === 'ダート') {
    if (dirtSires.some(s => bloodline.includes(s))) { /* [減点方式] potential += 25; */ /* [要見直し2] tags.push('ダートエリート血統'); */ }
  } else {
  ...
  const jraTopJockeys = ['ルメール', '川田', '武豊', '戸崎', '横山武', '松山', 'モレイラ', 'レーン'];
...
// [PERFORMANCE FIX]     tags.push(`💎期待値乖離(大化け期待)`);
  ...
  if (isJRA) {
    ...
    if (horse.pastRaces && horse.pastRaces.length >= 3) {
      ...
      if (avgPopularity <= 3.0 && lastFailed && isUnderValued) {
        // [減点方式] potential += 30;
// [PERFORMANCE FIX]         tags.push("⚡ 過去走人気トレンドからの巻き返し急襲穴馬");
      }
    }
  ...
  if (horse.pastRaces && horse.pastRaces.length >= 2) {
    ...
    if (bothBad && popularity > 4) {
      potential -= 25;
// [PERFORMANCE FIX]       tags.push("❌ 連続凡走ペナルティ: 直近2走連続8着以下×上位人気外");
    }
  }
  ...
  if (prevRaceData && prevRaceData.result >= 10 && popularity >= 5) {
    potential -= 15;
// [PERFORMANCE FIX]     tags.push("❌ 前走10着以下×4番人気以下ペナルティ");
  }
  ...
  if (prevRaceData && prevRaceData.jockeyWeight) {
    ...
    if (kinryoDiff >= 3) {
      potential -= 15;
// [PERFORMANCE FIX]       tags.push(`❌ 斤量急増ペナルティ(パフォーマンス低下リスク)`);
    }
  }
...
export function generateFormation(
  ...
  raceType: Formation['type'] = 'trifecta',
  race?: Race
): Formation {
  const sorted = [...predictions].sort(
    (a, b) => b.potential - a.potential || b.darkness - a.darkness || a.horseNumber - b.horseNumber
  );
  const horseNums = sorted.map(p => p.horseNumber);

  const oddsMap: Record<number, number> = {};
  if (race && race.horses) {
    race.horses.forEach(h => { oddsMap[h.number] = h.odds || 10; });
  } else {
    sorted.forEach((p, i) => {
  ...
  const horsesByOdds = (race?.horses || [])
    .map(h => ({ num: h.number, odds: h.odds || 100, horse: h }))
    .sort((a, b) => a.odds - b.odds);
  
  const getPopRank = (num: number) => {
    if (horsesByOdds.length === 0) {
      return sorted.findIndex(p => p.horseNumber === num) + 1;
    }
    const idx = horsesByOdds.findIndex(h => h.num === num);
    return idx === -1 ? 99 : idx + 1;
  };

  function calcSyntheticOdds(tix: number[][]): number {
    if (tix.length === 0) return 0;
    let sum = 0;
    for (const ticket of tix) {
      let est = 1;
      for (const n of ticket) est *= (oddsMap[n] || 10);
      if (ticket.length === 3) est = est / 4;
      else if (ticket.length === 2) est = est / 2;
      sum += 1 / Math.max(est, 1.1);
    }
    return Math.round((1 / sum) * 10) / 10;
  }

  const axisNos = horseNums.slice(0, 3);
  const darkNos = horseNums.slice(3, 7);

  let col1: number[] = [...axisNos];
  let col2: number[] | undefined = [...axisNos];
  let col3: number[] | undefined = undefined;
  let tickets: number[][] = [];
  let strategy = '';
  let riskLevel: 'safe' | 'normal' | 'risk' = 'normal';
  let stakeGuide = '';
  ...
  if (raceType === 'win') {
    ...
    let selectedPattern = 2;
    let wideTickets = new Set<string>();
    let wideStrategy = '';
    let wideStakeGuide = '推奨購入額: 1点あたり総資金の0.5〜1%\n（例: 資金10万円 → 500〜1,000円/点）\n\n【重要】ワイド購入時は必ず同じ買い目で「馬連」も少額併走し、ダブル的中を狙うこと。';

    const longShotTriggers = (race?.horses || []).filter(h => {
      const pop = getPopRank(h.number);
      if (pop < 10) return false;
      const isGelding = h.gender === 'セン';
      const isJockeyChange = h.prevJockey && h.jockey !== h.prevJockey;
      const useBlinkers = h.useBlinkers;
      const isTarget = predictions.find(p => p.horseNumber === h.number)?.targetTag;
      return isGelding || isJockeyChange || useBlinkers || isTarget;
    });

    const isHeavy = race?.condition === '重' || race?.condition === '不良';
    const firstPopHorse = horsesByOdds[0];
    const isFirstPopStrong = firstPopHorse && firstPopHorse.odds < 2.0;

    if (longShotTriggers.length > 0) {
      selectedPattern = 4;
      const triggerAxis = longShotTriggers.map(h => {
        const p = predictions.find(pr => pr.horseNumber === h.number);
        return { num: h.number, dark: p ? p.darkness : 0 };
      }).sort((a, b) => b.dark - a.dark)[0].num;

      const top3Pop = horsesByOdds.slice(0, 3).map(h => h.num);
      col1 = [triggerAxis];
      col2 = top3Pop;
      limitPoints = 3;
      top3Pop.forEach(t => {
        if (triggerAxis !== t) wideTickets.add([triggerAxis, t].sort((x,y) => x-y).join('-'));
      });
      wideStrategy = '【ワイド パターン④】大穴から上位人気への流し（秘密兵器）\n乗り替わりやブリンカーなど「一発で激走する条件」が揃った大穴から上位人気へ流す3点買い。高配当が狙える。';

    } else if (isHeavy) {
      const longShots = (race?.horses || []).filter(h => getPopRank(h.number) >= 5);
      const frontRunners = longShots.filter(h => ['逃げ', '先行', '好位'].includes(h.style));
      const closers = longShots.filter(h => ['差し', '中団', '後方', '追込', 'マクリ'].includes(h.style));
      
      const getGroupPot = (group: any[]) => group.reduce((sum, h) => {
        const p = predictions.find(pr => pr.horseNumber === h.number);
        return sum + (p ? p.potential : 0);
      }, 0);

      const targetGroup = getGroupPot(frontRunners) >= getGroupPot(closers) ? frontRunners : closers;
      const boxHorses = targetGroup.map(h => {
        const p = predictions.find(pr => pr.horseNumber === h.number);
        return { num: h.number, pot: p ? p.potential : 0 };
      }).sort((a, b) => b.pot - a.pot).slice(0, 4).map(x => x.num);
      ...
      if (boxHorses.length >= 2) {
        ...
        for (let i=0; i<boxHorses.length; i++) {
          for (let j=i+1; j<boxHorses.length; j++) {
            wideTickets.add([boxHorses[i], boxHorses[j]].sort((x,y) => x-y).join('-'));
          }
        }
    ...
    if (selectedPattern === 2 && isFirstPopStrong && horsesByOdds.length >= 4) {
      const axisCandidates = [horsesByOdds[1]?.num, horsesByOdds[2]?.num].filter(Boolean);
      const partnerCandidates = [horsesByOdds[0]?.num, horsesByOdds[1]?.num, horsesByOdds[2]?.num, horsesByOdds[3]?.num].filter(Boolean);
      
      let bestCombo: [number, number] | null = null;
      let maxOdds = 0;

      for (const a of axisCandidates) {
        for (const p of partnerCandidates) {
          if (a === p) continue;
          const estOdds = ((oddsMap[a] || 3) * (oddsMap[p] || 3)) / 2;
      ...
      if (bestCombo) {
        ...
        col2 = [bestCombo[1]];
        limitPoints = 1;
        wideTickets.add(bestCombo.sort((x,y) => x-y).join('-'));
        wideStrategy = '【ワイド パターン①】人気馬同士の1点勝負\n1番人気が圧倒的（1倍台）なレースで、あえて1番人気を外すか、オッズ5倍以上がつく人気馬同士の組み合わせを1点勝負。';
      }
    }

    if (selectedPattern === 2) {
      const axisNum = horsesByOdds[1]?.num || horseNums[0];
      const darkCandidates = (race?.horses || []).filter(h => {
        const pop = getPopRank(h.number);
        return pop >= 5 && pop <= 9;
      }).map(h => {
        const p = predictions.find(pr => pr.horseNumber === h.number);
        return { num: h.number, dark: p ? p.darkness : 0 };
      }).sort((a, b) => b.dark - a.dark).slice(0, 3).map(x => x.num);

      const finalDark = darkCandidates.length > 0 ? darkCandidates : [...sorted].sort((a,b) => b.darkness - a.darkness).filter(p => p.horseNumber !== axisNum).slice(0,3).map(p=>p.horseNumber);

      col1 = [axisNum];
      col2 = finalDark;
      limitPoints = 3;
      finalDark.forEach(d => {
        if (axisNum !== d) wideTickets.add([axisNum, d].sort((x,y) => x-y).join('-'));
      });
      wideStrategy = '【ワイド パターン②】人気馬から穴馬への流し\n信頼できる2番・3番人気を軸に、5番〜9番人気の穴馬2〜3頭へ流す。軸馬以外に気になる穴馬が複数いる場合に威力を発揮。';
    }

    tickets = Array.from(wideTickets).map(t => t.split('-').map(Number)).slice(0, limitPoints);
    ...
    if (tickets.length > limitPoints) {
      warningMessage = `ワイドは${limitPoints}点以内が原則。広げすぎると買った時点で外れる馬券が増えます。`;
      riskLevel = 'risk';
    }
